...
var validationMessages = function (method, arg, inverted) {
...
var getValidationMessage = validationMessages;
...
function validate(value, props) {
  const problems = Object.entries(props).filter((prop) => {
    if (!prop)
      return false;
    const ruleName = prop[0];
    if (!Object.values(Condition).includes(ruleName)) {
      throw new InvalidPropertyException(`Configuration property "${prop[0]}" is not supported.`, {});
    }
    return true;
  }).map(([conditionName, conditionValue]) => {
    const createValidationProblem = (condition, context) => {
      return {
        condition,
    ...
    switch (conditionName) {
      ...
      case Condition.HasLetterCount:
        {
          if ((Number.isNaN(conditionValue) || !Number.isInteger(conditionValue)) && typeof conditionValue !== "boolean") {
            throw new TypeError(`Configuration property "HasLetterCount" must be a number or boolean`);
          }
          const schema = new src();
      ...
      case Condition.IsLengthEqual:
        if (Number.isNaN(conditionValue) || !Number.isInteger(conditionValue)) {
          throw new TypeError(`Configuration property "IsLengthEqual" must be a number.`);
        }
        if ((typeof value === "string" || typeof value === "number") && value.toString().length !== conditionValue) {
          return createValidationProblem(conditionName, {
            requirement: conditionValue
          });
        }
      ...
      case Condition.IsLengthGreaterThanOrEqual:
        if (!conditionValue || Number.isNaN(conditionValue) || !Number.isInteger(conditionValue)) {
...
function getValidationProblemLabel(condition, context) {
  switch (condition) {
...
export { objectHasProperty, validate };
